...
//$body=str_ireplace("Дата","<abbr title=\"".$query."\">Дата</abbr>",$body);
$res=mssql_query($query);
while($row=mssql_fetch_row($res)) 
if (($row[6] > $row[5]) and ($row[6] - $row[5] > 30) ) {
	$query="SELECT SUM(CalWorksDec.TimeW) AS e1 FROM CalWorks INNER JOIN CalWorksDec ON CalWorks.ID_Rec = CalWorksDec.ID_Rec INNER JOIN CalWorksVarWorks ON CalWorksDec.ID_NW = CalWorksVarWorks.ID_NW
	...
	if ($row[6] - $row[5] - $r[0]*60 > 30) {
		$body.= "<tr class='tab_bg_1'>
			...
			<td>".$row[5]."</td>
			<td>".($r[0]*60)."</td>
			<td>".round(abs($row[6]-$row[5]-($r[0]*60)),1)."</td>";
